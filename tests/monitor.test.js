...
describe("Show all | Success Case", () => {
  test("Should send an object with keys data: [_id, name, image, manufacturer, size, price, stock] and howManyPages", (done) => {
    request(app)
      .get("/monitors")
      .query({ page: "1" })
      .end((err, res) => {
        ...
        expect(res.body.data[0]).toHaveProperty("size", expect.any(Number));
