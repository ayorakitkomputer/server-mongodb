...
describe("Show all | Success Case", () => {
  test("Should send an object with keys data: [_id, name, image, capacity, type, price, stock] and howManyPages", (done) => {
    request(app)
      .get("/storages")
      .query({ page: "1" })
      .end((err, res) => {
        ...
        expect(res.body.data[0]).toHaveProperty("capacity", expect.any(String));
        expect(res.body.data[0]).toHaveProperty("type", expect.any(String));
